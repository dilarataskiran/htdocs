...
@section('content')
 <div class="content-wrapper">
    ...
    <form action="{{ route('yonetici.siparis.kaydet', $sepet->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" readonly name="kullanici_id" value="{{ old('sepet_id',@$sepet->kullanici->adsoyad) }}" placeholder="Sepet ID">
                </div>
                </div>
              ...
              <div class="form-group">
                ...
                <select name="durum" class="form-control select2">
                    <option value="Beklemede" {{ $sepet->durum=="Beklemede" ? 'selected' : '' }}>Beklemede</option>
                    <option value="Onaylandı" {{ $sepet->durum=="Onaylandı" ? 'selected' : '' }}>Onaylandı</option>
                    <option value="Reddedildi" {{ $sepet->durum=="Reddedildi" ? 'selected' : '' }}>Reddedildi</option>
                    <option value="Kargoda" {{ $sepet->durum=="Kargoda" ? 'selected' : '' }}>Kargoda</option>
                    <option value="Teslim Edildi" {{ $sepet->durum=="Teslim Edildi" ? 'selected' : '' }}>Teslim Edildi</option>
                    <option value="İade Edildi" {{ $sepet->durum=="İade Edildi" ? 'selected' : '' }}>İade Edildi</option>
                </select>
              </div>
              </div>
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label>Alışveriş Toplamı</label>
                  <input type="text" class="form-control" required name="fiyat" readonly  value="{{ old('toplam',$sepet->toplam) }} TL" placeholder="Toplam">
                </div>
                </div>    
                ...
                <div class="col-md-3">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" required name="olusturma_tarihi" readonly  value="{{ date('Y-m-d',strtotime($sepet->olusturulma_tarihi))}}" placeholder="Toplam">
                    </div>
                    </div> 
            </div> 
        </div>
